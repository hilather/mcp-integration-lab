...
func serve(args []string) int {
	...
	mcpSrv := server.NewMCPServer("labinfo", "0.2.0",
		...
		server.WithInstructions("Lab service directory. Call endpoints_list for the user-facing web/REST URLs of every lab service (to direct users to the right place), and connections_list for protocol-level client configuration — hosts/ports, parameters like LDAP base/bind DNs, DNS zones, NFS mount options, SMTP settings, SSO issuer/JWKS — plus connection credentials when the lab runs in dev mode."),
	)
	...
	connectionsTool := mcp.NewTool("connections_list",
		mcp.WithDescription("Get the protocol-level connection details needed to configure a client or system under test against each lab service: protocol endpoints (SMTP, LDAP/LDAPS, DNS, NFS, TACACS+, RADIUS, NTP, HTTPS/OIDC/SAML, MCP) with host:port, client parameters (LDAP base/bind DNs and OUs, DNS zones, NFS mount options, AAA specifics, SSO issuer/JWKS), and the connection credentials (bind passwords, RADIUS/TACACS+ shared secrets, TacLab lab-user passwords, LabLDAP CA PEM, LabSSO CA PEM, IdP Alice password, tokens). Secrets are revealed only in dev mode. Tokens, passwords, and shared secrets come from dev-credentials.yaml when LAB_DEV_MODE=true; lab-ca, LabSSO CA, and TacLab client PEMs are this lab's generated material and are not catalog-stable. Otherwise each credential's usage explains what it is for and how to obtain it."),
