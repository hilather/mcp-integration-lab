...
3. Stop-the-line, effectiveness, and `capture-lesson` rules from that skill apply. A SHAPE / DIRECTION kick-back stops the line — do not “just fix it” on this PR. Emit the short replan with a failed-sweep autopsy (quoted blockers, what was never probed, cheapest experiment, what the next plan may not guess). After the loop, run `record-hint-outcome` if there is signal; otherwise say `no effectiveness signal`.
